defmodule ChatServer do
  ...
  def handle_cast({:enviar_mensaje, username, nombre_sala, mensaje}, estado) do
    ...
    if not Map.has_key?(estado.salas, nombre_sala) do
      ...
      # Solo enviar a usuarios que realmente están conectados
      usuarios_sin_entrega = []

      # Obtener el conjunto de miembros que también están conectados
      usuarios_sin_entrega = Enum.reduce(sala_actualizada.miembros, usuarios_sin_entrega, fn miembro, acc ->
        if Map.has_key?(nuevo_estado.usuarios_conectados, miembro) do
          # El usuario está conectado, intentar enviar el mensaje
          {pid, nodo, _} = nuevo_estado.usuarios_conectados[miembro]

          # Validar que el pid y el nodo sean válidos
        ...
        Elixir.ChatSession.procesar_mensajes_pendientes(pid, nombre_sala, username, nodo_servidor)#------------------------------------m
      end)
